/**
 * Email body parser - extracts lead data from email content.
 * Migrated from apps/web/src/lib/parser.ts
 */
...
function extractCustomerEmail(text: string): string | null {
  ...
  if (allEmails) {
    const customerEmail = allEmails.find(
      (e) =>
        !/(noreply|no-reply|mailer|system|wordpress|hostinger|shopify)/i.test(e),
    );
